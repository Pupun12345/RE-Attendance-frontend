...
class WorkersView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (initialSearchQuery != null) {
      controller.setInitialQuery(initialSearchQuery);
    }
    const themeBlue = Color(0xFF0B3B8C);

    // The Android system back gesture/button bypasses the AppBar's back
    // IconButton entirely, so it needs its own unfocus - same crash reason
    // as that button's onPressed above. canPop is false so we can unfocus
    // *before* popping (a callback that fires after the pop is too late -
    // the crash happens on the frame right after the widget is removed).
    return PopScope(
      canPop: false,
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
            ...
            // Dismiss the keyboard before popping the route. Otherwise, if the
            // search field still has focus, a pending keyboard-driven caret
            // animation callback can fire a frame later against a RenderObject
            // that's already been detached by the pop, crashing with
              ...
              Expanded(
                child: Obx(() {
                  ...
                  if (controller.errorMsg.value != null) {
                    return ListView(
                      children: [_WorkerRow(worker: controller.dummyWorker)],
                    );
                  }
                  ...
                  return RefreshIndicator(
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (_, i) =>
                          _WorkerRow(worker: controller.filteredWorkers[i]),
                    ),
                  );
                }),
              ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              // Same reasoning as the back button above - dismiss the
              // keyboard before navigating away from this screen.
              FocusScope.of(context).unfocus();
              Get.to(
                () => WorkerProfileView(
                  name: worker.name,
